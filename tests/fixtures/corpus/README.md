...
**measured in the real corpus** — the finding is named below so nobody simplifies one away.
...
and readable instead. The `cwd` **inside** each record is the substituted absolute path — that
...
| `normal/.claude/worktrees/orphan` | the same origin remote — the walk must **continue past** the exit-128 failure, not stop at it |
