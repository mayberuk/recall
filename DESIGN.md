...
share reasoning rather than a swatch. And **colour carries meaning, never decoration** — four inks,
...
- **Every pair is measured, both themes.** 14/14 verified at the last pass. Dark mode has failed
  twice while light passed, both times on a token that inverts lightness — check it separately.
...
### Coverage block
Ochre, and only ochre. Four real footer lines at 1:1, ghosted, one openable at a time — the open
...
*The ink lands.* `press` — `cubic-bezier(.16,.84,.3,1)`, overshoot then settle. Three instances:
...
watches the **section**, never the bars — a bar starts at zero height and a zero-height target
...
- Animate anything that is not the print head, the plotter, or a stamp — and never give two
...
into `--json`, `--format jsonl`, `--ids`, or the fzf record stream — those last two are parsed as
