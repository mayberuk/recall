...
Go implementation beside it, because holding assembly to a faster Go version lets a shared misreading
...
enough to build twice per run, and without that override it is under the threshold, and every case
...
versus 7.21 s with the standard library's `encoding/json`, a factor of 5.5, because `gjson` extracts the
