...
```
recall update            # verify the sha256, replace this binary
recall update --check    # report what is available, install nothing
```

recall makes **no network request of any kind** except from `recall update` and from `recall
doctor`, which refreshes a cached version number once a day. A search reads that cache and never
asks anybody, so there is still no daemon, no background process, and no cost to an answer.

When a newer release is cached, commands print one line about it on **stderr**, only to a terminal,
at most once a day. It never touches stdout, so it cannot land in a piped answer or an agent's
context. `RECALL_NO_UPDATE_CHECK` set to any non-empty value turns off both the check and the
notice. Full detail: [Staying current](https://recall.mayberuk.com/docs#staying-current).
