...
Code. Gemini CLI and Cursor are detected but have no reader yet, so a run that detects either falls
...
pass a flag, and the flag wins if both are set. `--provider` is a different flag from `--agent`:
